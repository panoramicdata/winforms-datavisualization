...
public partial class ChartGraphics
{
	...
	/// </summary>
	internal PointF _frontLinePoint1 = PointF.Empty;

	/// <summary>
	/// Point of the front line from the previous line segment.
	/// </summary>
	internal PointF _frontLinePoint2 = PointF.Empty;

	/// <summary>
	/// Previous line segment pen.
	/// </summary>
	internal Pen _frontLinePen = null;
		...
		)
	{
		...
		RectangleF pieTopRectangle = new()
		{
			X = topFirstRectPoint.X,
			Y = topFirstRectPoint.Y,
			Height = topSecondRectPoint.Y - topFirstRectPoint.Y,
			Width = topSecondRectPoint.X - topFirstRectPoint.X
		};

		RectangleF pieBottomRectangle = new()
		{
			X = bottomFirstRectPoint.X,
			Y = bottomFirstRectPoint.Y,
			Height = bottomSecondRectPoint.Y - bottomFirstRectPoint.Y,
			Width = bottomSecondRectPoint.X - bottomFirstRectPoint.X
		};
	...
	internal void FillPieSlice(ChartArea area, DataPoint point, SolidBrush brush, Pen pen, PointF firstRectPoint, PointF firstPoint, PointF secondRectPoint, PointF secondPoint, PointF center, float startAngle, float sweepAngle, bool fill, int pointIndex)
	{
		...
		// rectangle. This code will create RectangleF from
		// top left and bottom right points.
		RectangleF pieRectangle = new()
		{
			X = firstRectPoint.X,
			Y = firstRectPoint.Y,
			Height = secondRectPoint.Y - firstRectPoint.Y,
			Width = secondRectPoint.X - firstRectPoint.X
		};
	...
	internal void FillDoughnutSlice(ChartArea area, DataPoint point, SolidBrush brush, Pen pen, PointF firstRectPoint, PointF firstPoint, PointF secondRectPoint, PointF secondPoint, PointF threePoint, PointF fourPoint, float startAngle, float sweepAngle, bool fill, float doughnutRadius, int pointIndex)
	{
		...
		RectangleF pieDoughnutRectangle = new()
		{
			X = pieRectangle.X + pieRectangle.Width * (1F - doughnutRadius) / 2F,
			Y = pieRectangle.Y + pieRectangle.Height * (1F - doughnutRadius) / 2F,
			Height = pieRectangle.Height * doughnutRadius,
			Width = pieRectangle.Width * doughnutRadius
		};
		...
		SurfaceNames thinBorders)
	{
		...
		if (drawElements)
		{
			...
			if (thinBorders != 0)
			{
				Pen thinLinePen = new(surfaceBorderColor, 1);
		...
		{
			if (_frontLinePoint1 != PointF.Empty && _frontLinePen != null)
			{
				if ((_frontLinePoint1.X == polygonPoints[0].X &&
					_frontLinePoint1.Y == polygonPoints[0].Y ||
					_frontLinePoint2.X == polygonPoints[1].X &&
					_frontLinePoint2.Y == polygonPoints[1].Y) ||

					(_frontLinePoint1.X == polygonPoints[1].X &&
					_frontLinePoint1.Y == polygonPoints[1].Y ||
					_frontLinePoint2.X == polygonPoints[0].X &&
					_frontLinePoint2.Y == polygonPoints[0].Y) ||

					(_frontLinePoint1.X == polygonPoints[3].X &&
					_frontLinePoint1.Y == polygonPoints[3].Y ||
					_frontLinePoint2.X == polygonPoints[2].X &&
					_frontLinePoint2.Y == polygonPoints[2].Y) ||

					(_frontLinePoint1.X == polygonPoints[2].X &&
					_frontLinePoint1.Y == polygonPoints[2].Y ||
					_frontLinePoint2.X == polygonPoints[3].X &&
					_frontLinePoint2.Y == polygonPoints[3].Y))
				{
				...
				else
				{
					...
					DrawLine(
						_frontLinePen,
						(float)Math.Round(_frontLinePoint1.X),
						(float)Math.Round(_frontLinePoint1.Y),
						(float)Math.Round(_frontLinePoint2.X),
						(float)Math.Round(_frontLinePoint2.Y));
				}

				// Reset line properties
				_frontLinePen = null;
				_frontLinePoint1 = PointF.Empty;
				_frontLinePoint2 = PointF.Empty;
			}
			...
			{
				// Add top line
				_frontLinePen = thickBorderPen;
				_frontLinePoint1 = polygonPoints[0];
				_frontLinePoint2 = polygonPoints[1];
			}
		}
		...
		int yValueIndex)
	{
		...
		GraphicsPath splineSurfacePath = new();
		...
		bool clipInsideArea)
	{
		...
		//** Draw elements if required.
		//**********************************************************************
		Pen thinBorderPen = new(surfaceBorderColor, 1);
		if (drawElements)
		{
			// Draw the polygon
			...
			if (forceThinBorder || forceThickBorder)
			{
				if (forceThickBorder)
				{
					Pen linePen = new(surfaceBorderColor, borderWidth)
					{
						StartCap = LineCap.Round,
						EndCap = LineCap.Round
					};
		...
		if (area.Area3DStyle.Perspective == 0)
		{
			...
			if (drawElements)
			{
				_frontLinePen = (borderWidth > 1) ? thickBorderPen : thinBorderPen;
				_frontLinePoint1 = polygonPoints[0];
				_frontLinePoint2 = polygonPoints[1];
			}
		}
		...
		bool isVertical)
	{
		...
		else if (barDrawingStyle == BarDrawingStyle.LightToDark)
		{
			...
			using (GraphicsPath path = new())
			{
				...
				{
					using LinearGradientBrush topBrush = new(
							  bounds,
				...
				case (MarkerStyle.Circle):
					{
						if ((operationType & DrawingOperationTypes.DrawElement) == DrawingOperationTypes.DrawElement)
						{
							...
							Point3D[] centerPoint = [new(point.X, point.Y, positionZ + markerRelativeSize.Width)];
		...
		DrawingOperationTypes operationType)
	{
		...
		GraphicsPath bottomLeftSide = new();
						...
						case (SurfaceNames.Front):
							{
								...
								AddEllipseSegment(
									...
									(matrix.Perspective == 0) && veticalOrientation,
									cylinderAngle,
									out PointF leftOppSideLinePoint,
									out PointF rightOppSideLinePoint);
						...
						if (leftBottomPoint >= 0 && rigthTopPoint >= 0)
						{
							...
							if (closeToVertical)
							{
								if (ellipsePoints[leftBottomPoint].Y > oppositeEllipsePoints[_oppLeftBottomPoint].Y)
								{
									...
									(_oppRigthTopPoint, _oppLeftBottomPoint) = (_oppLeftBottomPoint, _oppRigthTopPoint);
								}
							}
...
public class Point3D
{
	...
	private PointF _coordXY = new(0f, 0f);
	...
	public PointF PointF
	{
		...
		set
		{
			_coordXY = new PointF(value.X, value.Y);
		}
	}
