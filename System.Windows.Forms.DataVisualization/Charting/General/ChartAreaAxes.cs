...
public partial class ChartArea
{
	...
	private void GetValuesFromData(Axis axis, out double autoMinimum, out double autoMaximum)
	{
		...
		string[] xAxesSeries = [.. GetXAxesSeries(type, axis.SubAxisName)];

		// Creates a list of series, which have same Y axis type.
		string[] yAxesSeries = [.. GetYAxesSeries(type, axis.SubAxisName)];
	...
	{
		return GetPointsInterval(Series, isLogarithmic, logarithmBase, false, out bool sameInterval);
	}
