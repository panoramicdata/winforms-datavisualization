...
internal class HotRegionsList : IDisposable
{
	...
	/// </summary>
	internal bool _hitTestCalled = false;
...
public class ChartElementOutline : IDisposable
{
	...
	{
		Markers = new ReadOnlyCollection<PointF>([]);
	}
		...
		, IDisposable
{
	...
	internal bool _enabledChecked = false;
	...
	internal string EvaluateToolTip(MouseEventArgs e)
	{
		...
		// Get tooltips from data points
		else if (type == ChartElementType.Title && obj != null && obj is Title title)
		{
			// Take tool tip from data point
	...
	internal HitTestResult[] HitTest(int x, int y, bool ignoreTransparent, params ChartElementType[] requestedElementTypes)
	{
		...
		for (int index = regionList.Count - 1; index >= 0; index--)
		{
			...
			if (!string.IsNullOrEmpty(alowedElements) && !alowedElements.Contains(region.Type.ToString() + ";"))
			{
				continue;
			}
		...
		object subObject)
	{
		...
		switch (type)
		{
			...
			case ChartElementType.AxisLabelImage:
				if (obj is CustomLabel label2)
				{
					result.Axis = label2.Axis;
					result.ChartArea = label2.Axis?.ChartArea;
				}
			...
			case ChartElementType.ScrollBarLargeDecrement:
				...
				result.Axis = scrollBar._axis;
				if (scrollBar._axis != null)
				{
					result.ChartArea = scrollBar._axis.ChartArea;
				}
