...
public partial class ChartArea
{
	...
	private float GetArea3DSceneDepth()
	{
		...
		bool indexedSeries = ChartHelper.IndexedSeries(Common, [.. Series]);
		...
		if (!indexedSeries)
		{
			clusteredInterval = GetPointsInterval(Series, xAxis.IsLogarithmic, xAxis.logarithmBase, false, out bool sameInterval, out smallestIntervalSeries);
		}
			...
			matrix3D.IsInitialized())
		{
			...
			if (!float.IsNaN(areaProjectionCenter.Z))
			{
				...
				for (int seriesIndex = 0; seriesIndex < seriesList.Count; seriesIndex++)
				{
					...
					GetSeriesZPositionAndDepth((Series)seriesList[seriesIndex], out float seriesDepth, out float seriesZPosition);
		...
		bool sideBySide)
	{
		...
		bool indexedSeries = ChartHelper.IndexedSeries(Common, [.. seriesNamesList]);
